...
#define PEC_SLEEP 0x58      // Error check code for sleep command
...
int32_t MlxReadObjectTemp(void)
{
    ...
    return temp[count++];
#else
    uint8_t bus_addr = (MLX_BUS_ADDR<<1) | 0;
...
void MlxSleep(void)
{
    ...
    uint8_t bus_addr = (MLX_BUS_ADDR<<1) | 0;
    uint8_t crc = crc8(0, bus_addr);
    crc = crc8(crc, ADDR_SLEEP);
    I2C1_Write1ByteRegister(bus_addr, ADDR_SLEEP, crc);
#endif    
